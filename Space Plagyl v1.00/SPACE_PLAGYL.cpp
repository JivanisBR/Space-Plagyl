...
void DrawStarShape(Vector2 center, float size, Color color)
{
    ...
    // --- SEÇÃO DAS DIAGONAIS REMOVIDA PARA MELHOR PERFORMANCE ---
    /*
    float diag_length = main_length * 0.7f;
    float o_c = diag_length * 0.707f;
    float o_w = width * 0.707f;

    Vector2 tip_tl = { center.x - o_c, center.y - o_c };
    Vector2 tip_br = { center.x + o_c, center.y + o_c };
    Vector2 shoulder_tr = { center.x + o_w, center.y - o_w };
    Vector2 shoulder_bl = { center.x - o_w, center.y + o_w };
    DrawTriangle(tip_tl, shoulder_tr, shoulder_bl, color);
    DrawTriangle(tip_br, shoulder_tr, shoulder_bl, color);

    Vector2 tip_tr = { center.x + o_c, center.y - o_c };
    Vector2 tip_bl = { center.x - o_c, center.y + o_c };
    Vector2 shoulder_br = { center.x + o_w, center.y + o_w };
    Vector2 shoulder_tl = { center.x - o_w, center.y - o_w };
    DrawTriangle(tip_tr, shoulder_br, shoulder_tl, color);
    DrawTriangle(tip_bl, shoulder_br, shoulder_tl, color);
    */
}
